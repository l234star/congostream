...
.glass-nav{position:fixed;top:0;left:0;right:0;z-index:99999;background:rgba(0,0,0,0.92);backdrop-filter:blur(15px);border-bottom:1px solid rgba(255,255,255,0.15);padding:8px 15px;display:flex;align-items:center;justify-content:space-between;}
.logo-text{color:#E50914;font-weight:900;font-size:22px;letter-spacing:1px;}
.genre-container{position:fixed;top:42px;left:0;right:0;z-index:99998;background:rgba(0,0,0,0.9);padding:8px 15px;display:flex;gap:8px;overflow-x:auto;white-space:nowrap;scrollbar-width:none;}
...
.glass-desc{background:rgba(15,15,15,0.8);backdrop-filter:blur(20px);border:1px solid rgba(255,255,255,0.18);border-radius:16px;padding:20px;margin-top:-30px;position:relative;z-index:5;max-width:650px;}
...
.publi-date{color:#888;font-size:12px;font-style:italic;}
    ...
    if os.path.exists(DATA_FILE):
        try:
            with open(DATA_FILE,"r") as f:
                st.session_state.films=json.load(f)
        except:
...
if "playing_film" not in st.session_state:
...
def save_films():
    with open(DATA_FILE,"w") as f:
        json.dump(st.session_state.films,f,indent=2,default=str)
...
if st.session_state.menu_open:
    choix = st.radio("", ["Accueil","Espace Associe","S'abonner"], label_visibility="collapsed")
    st.session_state.current_page = choix
    if st.button("Fermer X"):
        st.session_state.menu_open=False
        st.rerun()
...
if st.session_state.current_page == "Accueil":
    ...
    if sel:
        st.session_state.genre_filter = sel
    st.markdown('</div>', unsafe_allow_html=True)
...
if st.session_state.selected_film and menu == "Accueil":
    ...
    if film:
        ...
        if st.session_state.playing_film:
            if film.get("film_url"):
                video_auto(film["film_url"], "600px")
            elif film.get("trailer_url"):
                video_auto(film["trailer_url"], "600px")
        else:
            if film.get("trailer_url"):
                video_auto(film["trailer_url"], "600px")
            elif film.get("image_url"):
                st.image(film["image_url"], use_container_width=True)
            if st.button("LECTURE", use_container_width=True, type="primary"):
    ...
    if filtered:
        ...
        if hero.get("trailer_url"):
            video_auto(hero["trailer_url"], "550px")
        elif hero.get("image_url"):
            st.image(hero["image_url"], use_container_width=True)
        col1,col2 = st.columns([4,1])
        with col2:
            if st.button("Espace Associe", key="assoc_top"):
                st.session_state.current_page="Espace Associe"
                st.rerun()
        with col1:
            if st.button("LECTURE", key="hero_play", use_container_width=True, type="primary"):
                st.session_state.selected_film=hero["id"]
                st.session_state.playing_film=True
                st.rerun()
            if st.button("Film suivant", use_container_width=True):
    ...
    if CLOUD_OK:
        st.success("Cloudinary OK")
    else:
        st.error("Verifie Secrets")
    code = st.text_input("Code", type="password", placeholder="RolVie2002")
    if code and code!= "RolVie2002" and code!= "":
        st.error("Mauvais code")
        st.stop()
    if code == "RolVie2002":
        tab1, tab2 = st.tabs(["Publier Nouveau", "Gerer Contenus"])
        with tab1:
            with st.form("pub", clear_on_submit=True):
                ...
                affiche = st.file_uploader("Pochette / Affiche", type=["jpg","jpeg","png","webp"])
                    ...
                    if titre and trailer and CLOUD_OK:
                        with st.spinner("Upload..."):
                            ...
                            if t_url:
                                ...
                                save_films()
                                st.success(f"Publie le {now.strftime('%d/%m/%Y a %H:%M')}")
                                st.balloons()
                    else:
                        st.warning("Titre + Bande annonce obligatoire")
        with tab2:
            st.markdown(f"### {len(st.session_state.films)} films")
            if not st.session_state.films:
                ...
                for film in reversed(st.session_state.films):
                    ...
                    with col_a:
                        ...
                        date_affiche = film.get("timestamp") or film.get("date_pub","Date inconnue")
                        st.markdown(f'<div class="publi-date">Publie le {date_affiche}</div>', unsafe_allow_html=True)
                    with col_b:
                        if st.button("Modifier", key=f"edit_{film['id']}"):
                            st.session_state.edit_id=film["id"]
                    with col_c:
                        if st.button("Supprimer", key=f"del_{film['id']}", type="primary"):
                    ...
                    if st.session_state.edit_id==film["id"]:
                        with st.form(f"form_edit_{film['id']}"):
                            ...
                            with c2:
                                new_annee = st.text_input("Annee", value=film["annee"])
                            new_desc = st.text_area("Description", value=film.get("desc",""))
                            new_trailer = st.file_uploader("Nouvelle bande annonce", type=["mp4","mov","mkv"], key=f"new_trail_{film['id']}")
                            new_film = st.file_uploader("Nouveau film complet", type=["mp4","mkv"], key=f"new_film_{film['id']}")
                            new_affiche = st.file_uploader("Nouvelle pochette", type=["jpg","png","webp"], key=f"new_img_{film['id']}")
                            cs,ca = st.columns(2)
                            with cs:
                                if st.form_submit_button("Sauvegarder", type="primary", use_container_width=True):
                                    ...
                                    film["desc"]=new_desc
                                    if new_trailer:
                                        u = upload_cloud(new_trailer,"congo_trailers")
                                        if u:
                                            film["trailer_url"]=u
                                    if new_film:
                                        u = upload_cloud(new_film,"congo_films")
                                        if u:
                                            film["film_url"]=u
                                    if new_affiche:
                                        u = upload_cloud(new_affiche,"congo_images")
                                        if u:
                                            film["image_url"]=u
                                    save_films()
                                    st.session_state.edit_id=None
